...
namespace shared_model {
  namespace crypto {
    ...
    MultiBase::MultiBase(libp2p::multi::HashType hash_type,
                         const std::string &multi_blob)
        : MultiBase(hash_type,
                    Blob::Bytes{multi_blob.begin(), multi_blob.end()}) {}
        ...
        : libp2p::multi::Multihash([&] {
            ...
            return *iroha::expected::resultToOptionalValue(
                libp2p::multi::Multihash::create(
                    hash_type, kagome::common::Buffer{multi_blob}));
          }()),
          hex_([this] {
            auto hex_str = toHex();
    ...
    MultiBase::MultiBase(libp2p::multi::HashType hash_type,
                         const Blob &multi_blob)
        : MultiBase(hash_type, multi_blob.blob()) {}

    MultiBase MultiBase::fromHexString(libp2p::multi::HashType hash_type,
                                       const std::string &hex) {
      using iroha::operator|;
      MultiBase b(hash_type, "");
      iroha::hexstringToBytestring(hex) |
          [&](auto &&s) { b = MultiBase(hash_type, s); };
      return b;
    }
